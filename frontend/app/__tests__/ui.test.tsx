...
describe('SearchBar', () => {
  ...
  it('selecting an audio segment type updates the dropdown value', () => {
    render(<SearchBar onSearch={vi.fn()} />)
    fireEvent.click(screen.getByRole('button', { name: /toggle search filters/i }))
    const select = screen.getByRole('combobox', { name: /audio segment type/i }) as HTMLSelectElement
...
describe('SearchPage', () => {
  ...
  it('sends audio_segment_type when selected from the dropdown', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /toggle search filters/i }))
    fireEvent.change(screen.getByRole('combobox', { name: /audio segment type/i }), { target: { value: 'speech' } })
