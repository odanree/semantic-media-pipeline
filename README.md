...
- **RAG Ask**: Describe what you're looking for in plain English; the system retrieves semantically relevant frames, reads their captions, and returns a grounded answer with specific file paths and timestamps via GPT-4o-mini
- **Multi-Agent Coordinator**: LangGraph-based query router dispatches to visual search, metadata, or vision agents in parallel and aggregates results
- **Vision Captioning**: Every video frame gets a caption generated by a local VLM (llava:7b via Ollama) — no cloud, no data leaving the device
- **YOLO Object Detection**: On-demand object detection with confidence scores and bounding boxes for frame-level filtering
- **Audio Filters**: Search panel exposes "Has audio" and "Has speech" toggles — backed by DSP features extracted per video (RMS energy, speech band power, MFCC) stored directly in Qdrant payloads
- **Audit Middleware**: Every API request is logged to a Postgres `audit_logs` table with endpoint, method, status, and response time
...
- **Dual Storage Backends**: `local` mode mounts a filesystem volume; `s3` mode uses any S3-compatible store (Cloudflare R2, AWS S3, MinIO)
- **Proxy Sidecar**: Worker auto-generates 720p H.264/AAC faststart proxy files on ingest; browser streams the proxy by default for instant playback
...
| Layer | Technology |
|-------|-----------|
| **ML & Vision** | Sentence-Transformers CLIP (ViT-L-14, 768-dim), YOLOv8n, llava:7b (Ollama), FFmpeg, Pillow |
...
| **Intelligence** | LangGraph (multi-agent coordinator), RAG pipeline, GPT-4o-mini |
...
| `routers/ask.py` | 80% |
| `routers/agent.py` | 78% |
| `routers/detect.py` | 81% |
| `routers/stats.py` | 89% |
| `middleware/audit.py` | 85% |
| `utils.py` | 100% |
| **Total** | **82%** |
