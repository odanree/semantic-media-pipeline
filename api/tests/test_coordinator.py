...
class TestClassifyIntent:
    def test_visual_keywords_yield_visual_intent(self):
        from agents.query.coordinator import classify_intent
...
class TestNeedsVision:
    def test_empty_results_routes_to_vision(self):
        from agents.query.coordinator import needs_vision

        assert needs_vision({"search_results": []}) == "vision"

    def test_fewer_than_min_routes_to_vision(self):
        from agents.query.coordinator import needs_vision, MIN_RESULTS_FOR_VISION
...
class TestAgentNodes:
    def test_run_search_agent_populates_search_results(self):
        from agents.query.coordinator import run_search_agent

        mock_results = [{"file_path": "a.jpg", "similarity": 0.9}]
        with patch("agents.query.search_agent.search_agent_run", AsyncMock(return_value=mock_results)):
    ...
    def test_run_metadata_agent_populates_metadata_results(self):
        from agents.query.coordinator import run_metadata_agent
        import agents.query.metadata_agent  # ensure submodule is loaded before patching

        mock_results = [{"file_path": "b.jpg", "created_at": "2023-01-01"}]
        with patch("agents.query.metadata_agent.metadata_agent_run", AsyncMock(return_value=mock_results)):
    ...
    def test_run_search_agent_search_results_preserved_on_error(self):
        from agents.query.coordinator import run_search_agent
        import agents.query.search_agent  # ensure submodule is loaded before patching

        with patch("agents.query.search_agent.search_agent_run", AsyncMock(side_effect=RuntimeError("fail"))):
...
class TestAggregateAndAnswer:
    def test_empty_state_returns_no_media_string(self):
        ...
        mock_llm = MagicMock()
        with patch("agents.query.aggregator.get_llm_provider", return_value=mock_llm):
    ...
    def test_aggregate_and_answer_node_uses_build_final_answer(self):
        from agents.query.coordinator import aggregate_and_answer

        with patch(
            "agents.query.aggregator.build_final_answer",
